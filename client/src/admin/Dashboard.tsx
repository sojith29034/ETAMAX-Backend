...
const Dashboard = () => {
  ...
  const [transactions, setTransactions] = useState<{ payment: number }[]>([]);
    ...
    const fetchTransactions = async () => {
      try {
        ...
        const successTransactions = fetchedTransactions.filter(
          (transaction: { payment: number }) => transaction.payment === 1
        );
      ...
      <Row>
        <Col md={6}>
          <Table bordered hover variant='dark'>
            <thead className='text-center'>
              <tr><th colSpan={4}>Events with Seats Full - {fullEvents.length}</th></tr>
            </thead>
            <thead>
              <tr>
                <th>Event Name</th>
        ...
        <Col md={6}>
          <Table className='text-center' bordered variant='dark'>
            <thead>
              <tr><th colSpan={3}>Transaction Statistics</th></tr>
              <tr>
