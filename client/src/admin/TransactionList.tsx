...
const TransactionList = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const eventIds: string[] = transactionResponse.data.map(
          (transaction: Transaction) => transaction.eventId
        );
        ...
        const eventDetailsMap = eventResponses.reduce((acc, response) => {
          const eventData: Event = response.data as Event;
          acc[eventData._id] = {
            ...eventData,
            confirmedSeats: transactionResponse.data.filter(
              (transaction: Transaction) =>
                transaction.eventId === eventData._id &&
  ...
  const handleSelectAll = () => {
    ...
    setSelectedTransactions(
      selectAll ? [] : transactions.map((transaction) => transaction._id)
    );
  };
    ...
    if (transactionToConfirm) {
      ...
      if (event && event.confirmedSeats < event.maxSeats) {
        try {
          ...
          await axios.put(
            `${import.meta.env.VITE_BASE_URL}/api/transactions/${
              transactionToConfirm._id
            }`,
            updatedData
          );
          setTransactions(
            transactions.map((transaction) =>
              transaction._id === transactionToConfirm._id
                ? { ...transaction, payment: 1, transactionDate: formattedDate }
                : transaction
            )
          );
          setEventDetails({
        ...
        setShowAlert({
          message: "Cannot confirm payment: Event seats are full.",
          variant: "danger",
        });
        setShowConfirmModal(false);
        setTransactionToConfirm(null);
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setShowAlert({
        message: "Failed to delete selected transactions, please try again.",
        variant: "danger",
      });
    }
  };
  ...
  const filteredTransactions = transactions.filter(
    (transaction: Transaction) =>
      ...
      transaction.teamMembers.some((member) =>
  ...
  const exportToCSV = () => {
    ...
    const rows = transactions.map((transaction, index) => {
      ...
      if (transaction.teamMembers) {
        enrolledRollNumbers = transaction.teamMembers.reduce(
          (acc, rollNumber, i) => {
